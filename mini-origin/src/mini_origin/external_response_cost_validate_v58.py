...
def validate(reference_path: Path, rust_path: Path, output: Path) -> dict[str, object]:
    ...
    for expected in reference["rows"]:
        ...
        if actual is None:
            ...
            continue
        expected_solved = bool(expected["pareto_solved"])
        actual_solved = bool(actual["solved"])
        if expected_solved:
            solved_count += 1
        if expected_solved != actual_solved:
            mismatches.append({
                "digest": digest,
                "kind": "solved-status",
                "python": expected_solved,
                "rust": actual_solved,
            })
            continue
        if not expected_solved:
            exact_match_count += 1
            continue
        expected_plan = list(expected["pareto_plan"])
        if actual["plan"] != expected_plan:
            mismatches.append({
                "digest": digest,
                "kind": "plan",
                "python": expected_plan,
                "rust": actual["plan"],
            })
            continue
        expected_stats = expected["pareto_stats"]
        bad_fields = {
            field: {"python": expected_stats[field], "rust": actual[field]}
            for field in STAT_FIELDS
            if int(expected_stats[field]) != int(actual[field])
        }
        if bad_fields:
            mismatches.append({
                "digest": digest,
    ...
    unexpected = sorted(set(rust_by_digest) - {row["state_digest"] for row in reference["rows"]})
    if unexpected:
        mismatches.append({"kind": "unexpected-rust-rows", "digests": unexpected[:20]})

    profiled = int(reference["profiled_state_count"])
    pareto_solved = int(reference["pareto_solved_count"])
    ...
    gate = (
        ...
        and int(reference["base_state_count"]) >= 40
        and profiled >= 120
        and pareto_solved >= int(0.9 * profiled)
        and both >= 60
        and int(reference["plan_mismatch_count"]) == 0
        and pareto_only >= 15
        and not mismatches
        and exact_match_count == profiled
        and int(reference["dominated_queries_removed"]) >= 500
        and int(reference["root_incomparable_classes"]) > 0
        and median is not None
        and float(median) >= 3.0
        and p90 is not None
        and float(p90) >= 10.0
        and int(ladder["50000"]["pareto_solved"]) >= (
            int(ladder["50000"]["plain_solved"]) + 12
        )
    )
    result = {
        "status": (
            "external_response_cost_blind_pass"
            if gate else "external_response_cost_blind_rejected"
        ),
        "development_gate": gate,
        "claim_scope": (
            "Fresh official UCI datasets, frozen archive hashes, a matched plain exact "
            "solver, and an independently implemented Rust replay. A pass is strong "
            "external-data algorithmic evidence, not external peer review, publication "
            "novelty, or a world-class/world-first claim."
        ),
        ...
        "pareto_solved_count": pareto_solved,
        ...
        "rust_exact_match_count": exact_match_count,

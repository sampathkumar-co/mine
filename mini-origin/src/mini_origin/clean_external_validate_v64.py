...
def validate(reference_path: Path, rust_path: Path, output: Path) -> dict[str, object]:
    ...
    for expected in reference["rows"]:
        ...
        if actual["plan"] != list(expected["pareto_plan"]):
            mismatches.append({
                "digest": digest,
                "kind": "plan",
                "python": expected["pareto_plan"],
                "rust": actual["plan"],
            })
            continue
        bad = {
            field: {
                "python": expected["pareto_stats"][field],
                "rust": actual[field],
            }
            for field in STAT_FIELDS
            if int(expected["pareto_stats"][field]) != int(actual[field])
        }
        if bad:
    ...
    gate = (
        reference["parent_v60_digest"]
        == "fe0c56d83095bfc6607f9bfffb480ca829f369d6bd9db5ff47860184f7e246fb"
        and reference["repository_registry_digest"]
        == "870daa9b28450c1717a266ca73b7717ba24abba11b7f4c537e28e77ab1c0cc0d"
        and reference["archive_lock_digest"]
        == "5f48634b5e4f020b7dec6ec23b98141b4221f4495e3c1cfc8eb8ff44ba51609b"
    ...
    result = {
        "status": (
            "clean_external_conditioned_blind_pass"
            if gate
            else "clean_external_conditioned_blind_rejected"
        ),
        "development_gate": gate,
        "claim_scope": (
            "Repository-audited unused official UCI archives, a hash lock committed before record access, "
            "the unchanged v0.60 solver-independent generator, matched exact Python solvers and an "
            "independently implemented Rust replay. A pass is strong clean preregistered external-data "
            "algorithmic evidence, not outside-human reproduction, publication novelty, peer review, "
            "acceptance or a world-first/world-level claim."
        ),

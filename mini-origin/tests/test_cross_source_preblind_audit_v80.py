...
from mini_origin import cross_source_preblind_audit_v80 as audit
...
def test_preregistration_contains_no_candidate_metadata():
    ...
    assert row["status"] == "preregistered_before_new_source_metadata_access"
    assert row["new_source_candidate_ids_names_or_urls_committed_before_audit"] is False
    assert row["new_source_api_access_before_audit"] is False
    assert row["new_source_dataset_bytes_access_before_audit"] is False

...
def main():
    ...
    if len(rows)!=2900:raise RuntimeError(f'expected 2900 rows got {len(rows)}')
    ids={(r['train_manifest_name'],r['train_manifest_pointer_oid'],r['train_manifest_payload_sha256'],r['train_manifest_resolved_git_blob_sha1'],r['expected_test_manifest_name'],r['expected_test_manifest_pointer_oid'],int(r['expected_test_manifest_size']),r['source_sha256']) for r in rows}
    if len(ids)!=1:raise RuntimeError('manifest/source identity mismatch across shards')
    train_name,train_pointer,train_sha,train_blob,test_name,test_pointer,test_size,source_sha=next(iter(ids))
    keys=sorted({(r['arm'],r['candidate']) for r in rows},key=lambda x:(ARM_ORDER.index(x[0]),x[1]))
    if len(keys)!=29:raise RuntimeError(f'expected 29 candidates got {len(keys)}')
    summaries=[]
    for arm,name in keys:
        ...
        stable_fields=('rank','proposal_id','implementation_class','semantic_implementation_key','learned_template','baseline_id')
    ...
    ids_full=list(full.get('transfer_ids',[]));different_family_ids=[x for x in ids_full if source_by_id.get(x) and source_by_id[x]!=CURRENT_FAMILY];same_family_ids=[x for x in ids_full if source_by_id.get(x)==CURRENT_FAMILY]
    origins={x:source_by_id.get(x) for x in ids_full};nt_keys={x['semantic_implementation_key'] for x in summaries if x['arm']=='v6_no_transfer' and x['passes_training_correctness']};equivalent_available=full['semantic_implementation_key'] in nt_keys
    selected_pair_distinct=full['semantic_implementation_key']!=nt['semantic_implementation_key'];equal=full['valid']==nt['valid'] and full['invalid_output_retries']==nt['invalid_output_retries']
    ...
    comparison={'v6_full_harmonic':full['harmonic_speedup'],'v6_no_transfer_harmonic':nt['harmonic_speedup'],'strong_baseline_harmonic':strong['harmonic_speedup'],'v6_full_over_no_transfer_ratio':full_nt,'strong_baseline_time_over_full_time_harmonic':runtime_comp,'strong_baseline_competitiveness_threshold':0.80,'training_strong_baseline_competitiveness_passes':bool(strong['passes_training_correctness'] and runtime_comp>=0.80),'selected_v6_uses_learned_transfer':bool(ids_full),'selected_causal_ids':ids_full,'selected_different_family_transfer_ids':different_family_ids,'selected_same_family_transfer_ids':same_family_ids,'learned_recipe_source_families':origins,'current_family':CURRENT_FAMILY,'selected_v6_implementation_class':full['implementation_class'],'selected_v6_semantic_key':full['semantic_implementation_key'],'selected_no_transfer_implementation_class':nt['implementation_class'],'selected_no_transfer_semantic_key':nt['semantic_implementation_key'],'selected_pair_semantically_distinct':selected_pair_distinct,'equivalent_implementation_available_in_no_transfer':equivalent_available,'training_causal_separation_condition':sep,'training_baseline_qualified_causal_diagnostic':bool(full['passes_gate'] and different_family_ids and sep and strong['passes_training_correctness'] and runtime_comp>=0.80),'causal_transfer_credit':False,'reason_no_credit':'Training evidence only; causal credit is blind-only. Same-family transfer and equivalent no-transfer implementations receive zero causal credit.'}
    report={'campaign':'LEXIGEN v6 Applicability-Conditioned Causal Transfer Replication','task_index':7,'task':'svd','family':CURRENT_FAMILY,'stage':'official_training_r1','training_revision':'R1','training_records':100,'candidate_count':29,'candidate_evaluations':2900,'train_manifest_name':train_name,'train_manifest_pointer_oid':train_pointer,'train_manifest_payload_sha256':train_sha,'train_manifest_resolved_git_blob_sha1':train_blob,'expected_test_manifest_name':test_name,'expected_test_manifest_pointer_oid':test_pointer,'expected_test_manifest_size':test_size,'source_sha256':source_sha,'frozen_default_gate':{'valid_required':100,'harmonic_speedup_minimum':1.5,'minimum_speedup':1.05,'invalid_output_retries':0},'arms':arms,'all_candidates':summaries,'architecture_comparison':comparison,'blind_selection_ready':all(arms[k]['correct_candidate_count']>0 for k in ARM_ORDER),'official_test_manifest_contents_opened':False,'official_test_payloads_opened':0,'public_task_specific_solvers_opened':False,'threshold_changes':False,'scientific_candidate_changes':False,'verifier_changes':False}
    args.output.mkdir(parents=True,exist_ok=True);(args.output/'training-summary.json').write_text(json.dumps(report,indent=2)+'\n');ordered=sorted(rows,key=lambda r:(int(r['index']),ARM_ORDER.index(r['arm']),r['candidate']));(args.output/'training-results.jsonl').write_text('\n'.join(json.dumps(r,separators=(',',':')) for r in ordered)+'\n');print(json.dumps({'selected_by_arm':{k:v['selected'] for k,v in arms.items()},'comparison':comparison,'blind_selection_ready':report['blind_selection_ready']},indent=2))

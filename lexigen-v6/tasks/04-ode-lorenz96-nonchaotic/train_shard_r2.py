...
REVISION='bb02811fa47ca1c833baaa344949bcd8fb307ac8';TASK='ode_lorenz96_nonchaotic';SHARDS=10;EXPECTED_RECORDS=100
TRAIN_NAME='ode_lorenz96_nonchaotic_T100ms_n7856_size100_train.jsonl';TRAIN_POINTER_OID='0a1ba0bc39889f32559e0048d54293915b6deba2';TRAIN_SIZE=14552712;TRAIN_SHA256='babff21bed24a20606a3cdd774edb55af3114e7b8642395aacd519b73c7c6e4c'
TEST_NAME='ode_lorenz96_nonchaotic_T100ms_n7856_size100_test.jsonl';TEST_POINTER_OID='201c679a6268ae2f0ef32d75512f29ce9d503b73';TEST_SIZE=14553050;TEST_SHA256='4433959806e5674c416b32b7087c7ba0af9c70390e6690a0b7cdaafe6e026bb0'
BASE=f'https://huggingface.co/datasets/oripress/AlgoTune/resolve/{REVISION}/data/{TASK}'
SOURCE_SHA256='629456c15f57d932ded725c14d468bee2a513b97d124ad95d38401841bf68621'
...
def fetch(url:str)->bytes:
    ...
    for attempt in range(8):
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'LEXIGEN-v6-task4-train-r2'})
...
def decode_problem(raw):
    p=decode_value(raw)
    if not isinstance(p,dict) or not {'F','t0','t1','y0'} <= set(p):raise RuntimeError('invalid Lorenz96 problem structure')
    y0=np.asarray(p['y0'],dtype=np.float64);F=float(p['F']);t0=float(p['t0']);t1=float(p['t1'])
    if y0.ndim!=1 or len(y0)<4 or not np.all(np.isfinite(y0)) or not all(np.isfinite(x) for x in (F,t0,t1)) or t1<=t0:raise RuntimeError('invalid Lorenz96 problem values')
    return {'F':F,'t0':t0,'t1':t1,'y0':np.ascontiguousarray(y0,dtype=np.float64)}
...
def verify(problem,got,ref):
    try:
        obs=np.asarray(got,dtype=np.float64);exp=np.asarray(ref,dtype=np.float64);y0=np.asarray(problem['y0'],dtype=np.float64)
        if obs.shape!=y0.shape or exp.shape!=y0.shape or not np.all(np.isfinite(obs)):return False,'format_or_nonfinite',{}
        delta=np.abs(obs-exp);scale=np.maximum(np.abs(exp),1e-300)
        valid=bool(np.allclose(obs,exp,rtol=1e-5,atol=1e-8))
        return valid,(None if valid else 'official_allclose_mismatch'),{'max_abs_error':float(np.max(delta)),'max_rel_error':float(np.max(delta/scale)),'rtol':1e-5,'atol':1e-8}
...
def main():
    ...
    if len(manifest)!=TRAIN_SIZE or manifest_sha!=TRAIN_SHA256:raise RuntimeError(f'train payload identity mismatch size={len(manifest)} sha256={manifest_sha}')
    ...
    for idx,row in ((i,r) for i,r in enumerate(records) if i%SHARDS==args.shard):
        ...
        for c,got,cand_ns,error in runs:
            ...
            ev.append({'index':idx+1,'seed':int(row.get('seed',idx+1)),'arm':c.arm,'candidate':c.name,'implementation_class':c.implementation_class,'operators':list(c.operators),'transfer_ids':list(c.transfer_ids),'learned_template':c.learned_template,'baseline_id':c.baseline_id,'valid':bool(valid and error is None),'failure_reason':error or reason,'candidate_ns':cand_ns,'reference_ns':ref_ns,'speedup':(ref_ns/cand_ns) if cand_ns and cand_ns>0 else 0.0,**metrics,'state_dimension':int(len(p['y0'])),'F':float(p['F']),'t0':float(p['t0']),'t1':float(p['t1']),'train_manifest_name':TRAIN_NAME,'train_manifest_pointer_oid':TRAIN_POINTER_OID,'train_manifest_payload_sha256':manifest_sha,'train_manifest_resolved_git_blob_sha1':resolved_blob,'expected_test_manifest_name':TEST_NAME,'expected_test_manifest_pointer_oid':TEST_POINTER_OID,'expected_test_manifest_payload_sha256':TEST_SHA256,'expected_test_manifest_size':TEST_SIZE,'source_sha256':source_sha,'execution_order':execution_order,'shard':args.shard,'invalid_output_retries':0,'candidate_executions':1,'reference_executions_for_record':1,'test_manifest_contents_opened':False,'test_payloads_opened':0,'public_task_specific_solvers_opened':False,'training_revision':'R2_identity_only_repair'})

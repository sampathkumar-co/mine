...
TRAIN_NAME='ode_lorenz96_nonchaotic_T100ms_n7856_size100_train.jsonl';TRAIN_OID='0a1ba0bc39889f32559e0048d54293915b6deba2';TRAIN_SIZE=14552712
TEST_NAME='ode_lorenz96_nonchaotic_T100ms_n7856_size100_test.jsonl';TEST_OID='201c679a6268ae2f0ef32d75512f29ce9d503b73';TEST_SIZE=14553050
...
def fetch(url:str)->bytes:
    ...
    for attempt in range(8):
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'LEXIGEN-v6-task4-train-r1'})
...
def decode_problem(raw):
    ...
    if y0.ndim!=1 or len(y0)<4 or not np.all(np.isfinite(y0)) or not all(np.isfinite(x) for x in (F,t0,t1)) or t1<=t0:
        raise RuntimeError('invalid Lorenz96 problem values')
...
def verify(problem,got,ref):
    try:
        ...
        delta=np.abs(obs-exp);scale=np.maximum(np.abs(exp),1e-300);max_abs=float(np.max(delta));max_rel=float(np.max(delta/scale))
        valid=bool(np.allclose(obs,exp,rtol=1e-5,atol=1e-8))
        return valid,(None if valid else 'official_allclose_mismatch'),{'max_abs_error':max_abs,'max_rel_error':max_rel,'rtol':1e-5,'atol':1e-8}
...
def main():
    ...
    blob=git_blob(manifest)
    if len(manifest)!=TRAIN_SIZE or blob!=TRAIN_OID:raise RuntimeError(f'train manifest identity mismatch size={len(manifest)} blob={blob}')

...
VALID_REQUIRED=100;HARMONIC_REQUIRED=1.50;MINIMUM_REQUIRED=1.05;CURRENT_FAMILY='scientific_computing'
...
def semantic_key(impl:str)->str:
    if impl in {'source_equivalent_transfer_fallback','scipy_rk45_reference'}:return 'scipy_rk45_reference'
    if impl=='preindexed_rk45_exact':return 'preindexed_rk45_exact'
    if impl=='sliced_cyclic_rk45_exact':return 'sliced_cyclic_rk45_exact'
    return impl
...
def main():
    ...
    ids={(r['train_manifest_name'],r['train_manifest_pointer_oid'],r['train_manifest_payload_sha256'],r['train_manifest_resolved_git_blob_sha1'],r['expected_test_manifest_name'],r['expected_test_manifest_pointer_oid'],r['expected_test_manifest_payload_sha256'],int(r['expected_test_manifest_size']),r['source_sha256']) for r in rows}
    if len(ids)!=1:raise RuntimeError('manifest/source identity mismatch across shards')
    train_name,train_pointer,train_sha,train_blob,test_name,test_pointer,test_sha,test_size,source_sha=next(iter(ids))

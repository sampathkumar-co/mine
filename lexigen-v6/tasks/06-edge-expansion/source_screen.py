...
def main():
    ...
    body={'campaign':'LEXIGEN v6 Applicability-Conditioned Causal Transfer Replication','task_index':6,'task':'edge_expansion','family':'miscellaneous','stage':'source_screen_r2_serialization_only_repair','repair_from_run_id':32934721936,'source_sha256':sha,'features':list(fp.features),'dependency_calls':list(fp.dependency_calls),'applicable_transfer_templates':app,'campaign_preregistered_applicable_transfer_ids':list(CAMPAIGN_IDS),'arms':arms,'arm_counts':{k:len(v) for k,v in arms.items()},'official_train_manifest_opened':False,'official_test_manifest_opened':False,'official_payloads_opened':0,'public_task_specific_solvers_opened':False}

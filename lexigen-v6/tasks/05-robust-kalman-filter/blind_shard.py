...
from candidates import build_candidates,high_accuracy_independent,independent_semantic_certificate,source_reference,verify_against_reference
from train_shard import BASE,SHARDS,EXPECTED_RECORDS,TEST_NAME,TEST_OID,TEST_SIZE,SOURCE_SHA256,decode_problem,fetch,git_blob,timed
...
def load_lock()->dict:
    ...
    if lock.get('task')!='robust_kalman_filter' or lock.get('stage')!='blind_r1_lock':raise RuntimeError('invalid blind lock')
...
def selected_candidates(source_text:str,lock:dict):
    ...
    if abl['resulting_transfer_ids']!=[]:raise RuntimeError('ablation transfer IDs must be empty')
    if abl['retained_operators']!=list(full.operators):raise RuntimeError('ablation operator mismatch')
    if abl['resulting_implementation_class']!=full.implementation_class:raise RuntimeError('ablation implementation mismatch')
    if not abl['same_callable_as_full'] or not abl['preblind_semantic_equivalence_to_full']:raise RuntimeError('preblind ablation equivalence not acknowledged')
    return selected,full


def evaluation_entries(selected,full,lock):
    rows=[]
    for c in selected:
...
def run_smoke(source_text:str,lock:dict)->None:
    ...
    for case_index,p in enumerate(cases):
        ...
        for entry in entries:
            got=entry['solve'](p);ok,reason,_=verify_against_reference(p,got,ref,objective_factor=1.01,eps=1e-5);sem,_=independent_semantic_certificate(p,got,ind)
            if not ok or not sem:raise RuntimeError(f'smoke invalid case={case_index} candidate={entry["candidate"]} reason={reason} semantic={sem}')
            valid+=1
...
def run_official(shard:int,output:Path,source_text:str,source_sha:str,lock:dict)->None:
    ...
    manifest=fetch(f'{BASE}/{TEST_NAME}?download=true');blob=git_blob(manifest)
    if len(manifest)!=TEST_SIZE or blob!=TEST_OID:raise RuntimeError(f'test manifest identity mismatch size={len(manifest)} blob={blob}')
    ...
    for idx,row in ((i,r) for i,r in enumerate(records) if i%SHARDS==shard):
        ...
        if idx%2==0:
            ref,ref_ns,ref_err=timed(source_reference,p);runs=[(e,*timed(e['solve'],p)) for e in ordered];execution_order='reference_first'
        else:
            runs=[(e,*timed(e['solve'],p)) for e in ordered];ref,ref_ns,ref_err=timed(source_reference,p);execution_order='candidates_first'

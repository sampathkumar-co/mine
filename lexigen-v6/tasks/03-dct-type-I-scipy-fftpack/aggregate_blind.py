...
def main():
    ...
    causal_conditions={'full_passes_clean_blind_gate':bool(full['passes_clean_blind_gate']),'selected_full_uses_learned_transfer':selected_full_uses_learned,'selected_full_uses_different_family_transfer':bool(selected_diff_ids),'selected_full_no_transfer_semantically_distinct':selected_pair_distinct,'source_family_differs_from_current':source_family_differs,'causal_separation':bool(causal_separation),'recipe_removal_eliminates_qualifying_advantage':bool(recipe_removal_eliminates),'strong_baseline_valid_same_denominator':bool(baseline_valid),'strong_baseline_competitiveness_passes':bool(baseline_pass)}
    ...
    summary={'campaign':'LEXIGEN v6 Applicability-Conditioned Causal Transfer Replication','task_index':3,'task':'dct_type_I_scipy_fftpack','family':'signal_processing','stage':'official_blind_r1','blind_records':100,'blind_entries':7,'candidate_evaluations':len(rows),'test_manifest_name':test_name,'test_manifest_git_blob_sha1':test_oid,'test_manifest_sha256':test_sha,'source_sha256':source_sha,'frozen_clean_gate':{'valid_required':100,'harmonic_speedup_minimum':1.50,'minimum_speedup':1.05,'invalid_output_retries':0},'by_arm':by_arm,'clean_wins':clean_wins,'full_no_transfer_harmonic_ratio':full_nt,'full_recipe_removal_harmonic_ratio':full_abl,'strong_baseline_time_over_full_time_harmonic':baseline_comp,'causal_conditions':causal_conditions,'baseline_qualified_causal_transfer_win':causal_win,'causal_transfer_detected_but_baseline_uncompetitive':False,'preblind_recipe_removal_equivalence_flag':bool(lock['recipe_removal_ablation']['preblind_semantic_equivalence_to_full']),'same_family_transfer_selected':bool(lock['causal_preblind_diagnostics']['selected_same_family_transfer_ids']),'task3_causal_win_impossible_preblind':bool(lock['causal_preblind_diagnostics']['task3_causal_win_impossible_preblind']),'invalid_output_retries_total':sum(int(r['invalid_output_retries']) for r in rows),'verifier_capacity_loophole_exploited':False,'results_sha256':hashlib.sha256(payload.encode()).hexdigest(),'blind_run_complete':True,'post_blind_candidate_revision_allowed':False,'post_blind_timing_rerun_allowed':False}

...
def load_lock() -> dict:
    ...
    if lock.get("task") != "max_flow_min_cost" or lock.get("stage") != "blind_r1_lock":
...
def selected_candidates(source_text: str, lock: dict):
    ...
    if ablation["resulting_implementation_class"] != full.implementation_class:
        raise RuntimeError("frozen ablation implementation mismatch")
    if not ablation["same_callable_as_full"] or not ablation["preblind_semantic_equivalence_to_full"]:
        raise RuntimeError("Task1 frozen recipe-removal equivalence not acknowledged")

...
VALID_REQUIRED=100;HARMONIC_REQUIRED=1.50;MINIMUM_REQUIRED=1.05;CURRENT_FAMILY='miscellaneous'
...
def harmonic(v):return len(v)/sum(1.0/x for x in v) if v and all(x>0 for x in v) else 0.0

def semantic_key(impl:str)->str:
    if impl in {'precision_budgeted_lowlevel_odr','lowlevel_odrpack_direct_weights'}:return 'lowlevel_odrpack_direct_weights'
    if impl in {'source_equivalent_fallback','scipy_odr_reference'}:return 'scipy_odr_reference'
    return impl
...
def main():
    ...
    if len(rows)!=2700:raise RuntimeError(f'expected 2700 rows got {len(rows)}')
    ids={(r['train_manifest_name'],r['train_manifest_git_blob_sha1'],r['train_manifest_sha256'],r['expected_test_manifest_name'],r['expected_test_manifest_tree_oid'],int(r['expected_test_manifest_size']),r['source_sha256']) for r in rows}
    ...
    if len(keys)!=27:raise RuntimeError(f'expected 27 candidates got {len(keys)}')
    summaries=[]
    ...
    sep=bool(cid and selected_pair_distinct and not equivalent_available and full['passes_training_correctness'] and ((not nt['passes_gate']) or (equal and full_nt>=1.25)))
    comparison={'v6_full_harmonic':full['harmonic_speedup'],'v6_no_transfer_harmonic':nt['harmonic_speedup'],'strong_baseline_harmonic':strong['harmonic_speedup'],'v6_full_over_no_transfer_ratio':full_nt,'strong_baseline_time_over_full_time_harmonic':runtime_comp,'strong_baseline_competitiveness_threshold':0.80,'training_strong_baseline_competitiveness_passes':bool(strong['passes_training_correctness'] and runtime_comp>=0.80),'selected_v6_uses_learned_transfer':cid is not None,'selected_causal_id':cid,'learned_recipe_source_family':origin,'current_family':CURRENT_FAMILY,'source_family_differs_from_current':source_diff,'selected_v6_implementation_class':full['implementation_class'],'selected_v6_semantic_key':full['semantic_implementation_key'],'selected_no_transfer_implementation_class':nt['implementation_class'],'selected_no_transfer_semantic_key':nt['semantic_implementation_key'],'selected_pair_semantically_distinct':selected_pair_distinct,'equivalent_implementation_available_in_no_transfer':equivalent_available,'training_causal_separation_condition':sep,'training_baseline_qualified_causal_diagnostic':bool(full['passes_gate'] and cid and source_diff and sep and strong['passes_training_correctness'] and runtime_comp>=0.80),'causal_transfer_credit':False,'reason_no_credit':'Training evidence only; v6 causal credit is blind-only and requires frozen recipe-removal ablation.'}
    report={'campaign':'LEXIGEN v6 Applicability-Conditioned Causal Transfer Replication','task_index':2,'task':'odr','family':CURRENT_FAMILY,'stage':'official_training_r1','training_records':100,'candidate_count':27,'candidate_evaluations':2700,'train_manifest_name':train_name,'train_manifest_git_blob_sha1':train_oid,'train_manifest_sha256':train_sha,'expected_test_manifest_name':test_name,'expected_test_manifest_tree_oid':test_oid,'expected_test_manifest_size':test_size,'source_sha256':source_sha,'frozen_default_gate':{'valid_required':100,'harmonic_speedup_minimum':1.5,'minimum_speedup':1.05,'invalid_output_retries':0},'arms':arms,'all_candidates':summaries,'architecture_comparison':comparison,'blind_selection_ready':all(arms[k]['correct_candidate_count']>0 for k in ARM_ORDER),'official_test_manifest_contents_opened':False,'official_test_payloads_opened':0,'threshold_changes':False}

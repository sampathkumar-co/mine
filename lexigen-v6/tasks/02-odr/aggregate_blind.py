...
def summarize(rows: list[dict]) -> dict:
    ...
    out = {
        ...
        "semantic_implementation_key": rows[0]["semantic_implementation_key"],
...
def main() -> None:
    ...
    equal_nt = full["valid"] == no_transfer["valid"] and full["invalid_output_retries"] == no_transfer["invalid_output_retries"]
    full_nt_ratio = full["harmonic_speedup"] / no_transfer["harmonic_speedup"] if no_transfer["harmonic_speedup"] > 0 else math.inf
    causal_separation = (not no_transfer["passes_clean_blind_gate"]) or (full_nt_ratio >= 1.25 and equal_nt)

    equal_ablation = full["valid"] == ablation["valid"] and full["invalid_output_retries"] == ablation["invalid_output_retries"]
    full_ablation_ratio = full["harmonic_speedup"] / ablation["harmonic_speedup"] if ablation["harmonic_speedup"] > 0 else math.inf
    recipe_removal_eliminates_advantage = (not ablation["passes_clean_blind_gate"]) or (full_ablation_ratio >= 1.25 and equal_ablation)

    baseline_valid_same_denominator = strong["records"] == 100 and strong["valid"] == 100 and strong["invalid_output_retries"] == 0
    baseline_competitiveness = full["harmonic_speedup"] / strong["harmonic_speedup"] if strong["harmonic_speedup"] > 0 else math.inf
    ...
    pre = lock["causal_preblind_diagnostics"]
    selected_pair_semantically_distinct = bool(pre["selected_full_no_transfer_semantically_distinct"])
    source_family_differs = bool(pre["source_family_differs_from_current"])
    ...
    summary = {
        ...
        "full_no_transfer_harmonic_ratio": full_nt_ratio,
        ...
        "preblind_equivalent_implementation_available_in_no_transfer": bool(pre["equivalent_implementation_available_in_no_transfer"]),
        "preblind_recipe_removal_semantic_equivalence_to_full": bool(lock["recipe_removal_ablation"]["preblind_semantic_equivalence_to_full"]),
    ...
    print(json.dumps({
        ...
        "full_no_transfer_ratio": full_nt_ratio,
        "full_recipe_removal_ratio": full_ablation_ratio,
        "baseline_qualified_causal_transfer_win": baseline_qualified_causal_transfer_win,

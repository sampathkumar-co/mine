...
def summarize(rows):
    ...
    out={
        "arm":rows[0]["arm"],"candidate":rows[0]["candidate"],"implementation_class":rows[0]["implementation_class"],"operators":rows[0]["operators"],"transfer_ids":rows[0]["transfer_ids"],"learned_template":rows[0]["learned_template"],"baseline_id":rows[0]["baseline_id"],
        "records":len(rows),"valid":valid,"invalid_outputs":len(rows)-valid,"invalid_output_retries":retries,
        ...
        "shared_execution_class":all(bool(r.get("shared_execution_class")) for r in rows),
    }
    out["passes_frozen_performance_gate"]=(out["records"]==100 and out["valid"]==100 and retries==0 and out["harmonic_speedup"]>=1.50 and out["minimum_speedup"]>=1.05)
    return out


def choose(rows):
    return sorted(rows,key=lambda s:(-int(s["valid"]==100),-int(s["passes_frozen_performance_gate"]),-s["harmonic_speedup"],-s["minimum_speedup"],s["candidate"]))[0]
...
def main():
    ...
    if len(grouped)!=EXPECTED_CANDIDATES: raise RuntimeError(f"{args.task}: expected 31 candidates got {len(grouped)}")
    if any(len(v)!=EXPECTED_RECORDS for v in grouped.values()): raise RuntimeError(f"{args.task}: candidate denominator mismatch")
    summaries=[summarize(group) for group in grouped.values()]
    by_arm={}
    for arm in ARM_ORDER:
        candidates=[s for s in summaries if s["arm"]==arm]
        expected=1 if arm=="strong_baseline" else 6
        if len(candidates)!=expected: raise RuntimeError(f"{args.task}: arm {arm} expected {expected} candidates got {len(candidates)}")
    ...
    if len(manifest_hashes)!=1 or len(source_hashes)!=1 or len(manifest_names)!=1 or len(test_names)!=1: raise RuntimeError("identity mismatch across shards")
    summary={
        "campaign":"LEXIGEN v6 Applicability-Conditioned Causal Transfer Replication","stage":"official_training_r1_parallel7_sealed","task":args.task,"task_index":rows[0]["task_index"],"family":rows[0]["family"],
        "training_records":100,"candidate_count":31,"candidate_evaluations":len(rows),"unique_implementation_classes":len({s["implementation_class"] for s in summaries}),
        "execution_deduplication_by_frozen_implementation_class":True,"train_manifest_name":next(iter(manifest_names)),"train_manifest_sha256":next(iter(manifest_hashes)),"expected_test_manifest_name":next(iter(test_names)),"official_test_manifest_opened":False,"official_test_payloads_opened":0,"source_sha256":next(iter(source_hashes)),
        "frozen_gate":{"valid_required":100,"harmonic_speedup_minimum":1.50,"minimum_speedup":1.05,"invalid_output_retries":0},
        "selected_by_arm":by_arm,"all_candidate_summaries":sorted(summaries,key=lambda s:(s["arm"],s["candidate"])),
        "training_diagnostics":{"v6_full_over_no_transfer_harmonic_ratio":full_nt_ratio,"selected_full_uses_learned_transfer":bool(full["transfer_ids"]),"selected_full_no_transfer_implementation_class_equal":full["implementation_class"]==nt["implementation_class"],"selected_full_no_transfer_semantically_distinct":full["implementation_class"]!=nt["implementation_class"],"strong_baseline_time_over_full_time_harmonic":baseline_ratio,"full_passes_training_performance_gate":bool(full["passes_frozen_performance_gate"])},
        "candidates_passing_training_performance_gate":sum(bool(s["passes_frozen_performance_gate"]) for s in summaries),"results_sha256":hashlib.sha256(payload.encode()).hexdigest(),"scientific_candidate_revision_after_training":False,"threshold_changes":False,
    }

...
from parallel_candidates import ARM_ORDER, Candidate
from parallel_train import EXPECTED_RECORDS, REVISION, SHARDS, SOURCE_COMMIT, TASKS

ABLATION_ARM="recipe_removal_ablation"
REPAIRED_TASKS={"ode_lorenz96_nonchaotic","edge_expansion"}


def activate_task_overlay(task: str) -> None:
    if task in REPAIRED_TASKS:
...
def selected_entries(task: str, source_text: str):
    ...
    ablation=Candidate(name=f"recipe_removal_{full.name}",arm=ABLATION_ARM,implementation_class=abl_impl,operators=tuple(full.operators),transfer_ids=(),learned_template=None,baseline_id=None,solve=abl_fn)
    return summary,selected+[ablation]


def preblind_smoke(task,entries):
    activate_task_overlay(task)
    class_reps={}
    for c in entries: class_reps.setdefault(c.implementation_class,c)
    rows=[]
    for case_idx,problem in enumerate(pt.synthetic_problems(task),1):
...
def main():
    ...
    if lock.get("task_count")!=7: raise RuntimeError("parallel7 blind task count mismatch")
    verify_all_training_summaries_against_lock(lock)
    activate_task_overlay(task)
    src_raw=pt.fetch(f"https://raw.githubusercontent.com/oripress/AlgoTune/{SOURCE_COMMIT}/{cfg['source_path']}"); src_sha=hashlib.sha256(src_raw).hexdigest()
    ...
    if train_summary.get("official_test_manifest_opened") is not False or int(train_summary.get("official_test_payloads_opened",-1))!=0: raise RuntimeError(f"{task}: training summary crossed test boundary")
    test_name=str(train_summary["expected_test_manifest_name"])
    if "/" in test_name or ".." in test_name or not test_name.endswith("_test.jsonl") or not test_name.startswith(task+"_"): raise RuntimeError(f"{task}: unsafe sealed test manifest name {test_name}")
    expected_test_meta=train_summary.get("expected_test_manifest_tree_metadata",{})
    smoke=preblind_smoke(task,entries)
    ...
    manifest=pt.fetch(f"{base}/{test_name}?download=true",ua="LEXIGEN-v6-parallel7-blind-r1")
    if expected_test_meta.get("size") is not None and len(manifest)!=int(expected_test_meta["size"]): raise RuntimeError(f"{task}: sealed test manifest size mismatch {len(manifest)} != {expected_test_meta['size']}")
    manifest_sha=hashlib.sha256(manifest).hexdigest(); records=[json.loads(line) for line in manifest.decode("utf-8").splitlines() if line.strip()]
    ...
    for idx,row in ((i,r) for i,r in enumerate(records) if i%SHARDS==args.shard):
        problem=pt.decode_value(row["problem"],base); classes=list(reps); shift=idx%len(classes); classes=classes[shift:]+classes[:shift]
        if idx%2==0:
            ref,ref_ns,ref_err=pt.timed(pc.REFERENCE_SOLVERS[task],problem); class_runs={impl:pt.timed(reps[impl].solve,problem) for impl in classes}; execution_order="reference_first"
        else:
            class_runs={impl:pt.timed(reps[impl].solve,problem) for impl in classes}; ref,ref_ns,ref_err=pt.timed(pc.REFERENCE_SOLVERS[task],problem); execution_order="classes_first"
    ...
    print(json.dumps({"task":task,"shard":args.shard,"rows":len(evidence),"implementation_classes":sorted(reps),"preblind_smoke_sha256":smoke_sha,"sealed_test_manifest_name":test_name,"test_manifest_sha256":manifest_sha,"task46_repair_overlay_active":task in REPAIRED_TASKS},indent=2))

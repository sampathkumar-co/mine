...
def download(url: str, destination: Path) -> None:
    ...
    for attempt in range(3):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v2-train"})
...
def warm_up() -> None:
    problem: Problem = {
        ...
        "plaintext": b"LEXIGEN warm-up" * 64,
        "associated_data": b"training-only warm-up",
    }
    ref = execute(reference, problem)
    if ref["error"] is not None:
        raise RuntimeError(str(ref["error"]))
    for name, candidate in CANDIDATES.items():
        result = execute(candidate, problem)
        if result["error"] is not None or result["fingerprint"] != ref["fingerprint"]:
...
def main() -> None:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "LEXIGEN-v2-train"},
    )
    ...
    for index, row in selected:
        ...
        for name in names:
            ...
            print(
                f"[{index + 1}/100] {name} valid={valid} candidate={candidate_s!s}s "
                f"reference={reference_s:.6f}s speedup={speedup:.3f}", flush=True
            )

...
def main() -> None:
    ...
    for candidate in candidates:
        ...
        if len(selected_rows) != 100:
            raise RuntimeError(f"{candidate} has {len(selected_rows)} rows")
        speeds = [float(row["speedup"]) for row in selected_rows]
        summaries.append({
            "candidate": candidate,
            "count": 100,
            "valid": sum(bool(row["valid"]) for row in selected_rows),
            "exceptions": sum(
            ...
            "median_reference_s": statistics.median(
                float(row["reference_s"]) for row in selected_rows
            ),
            "edge_count_minimum": min(int(row["edge_count"]) for row in selected_rows),
            "edge_count_maximum": max(int(row["edge_count"]) for row in selected_rows),
            "articulation_count_minimum": min(int(row["articulation_count"]) for row in selected_rows),
            "articulation_count_maximum": max(int(row["articulation_count"]) for row in selected_rows),
        })
    ...
    report = {
        ...
        "train_manifest_name": "articulation_points_T100ms_n837_size100_train.jsonl",
        "train_manifest_sha256": "a12a2dbce09a1e91a9790c209709a374375501589a42d92a8930a99edb5d8a6f",
        "expected_test_manifest_name": "articulation_points_T100ms_n837_size100_test.jsonl",
        "expected_test_manifest_git_oid": "197846cce9b36500444003123dab29e577fd598e",
        "selected": selected,

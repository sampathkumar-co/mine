...
def request_bytes(url: str) -> bytes:
    ...
    for delay in (0, 5, 15, 30, 60):
        if delay:
            time.sleep(delay)
        request = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v3-procrustes-r2-train"})
        try:
...
def main() -> None:
    ...
    modes = {"adaptive_cast_gesdd": "cast", "adaptive_product_gesdd": "product"}
    with tempfile.TemporaryDirectory() as directory:
        ...
        for index, row in selected:
            ...
            for name, solution, candidate_s, candidate_error in candidate_results:
                ...
                with threadpool_limits(limits=1):
                    ratio = diagnostic_ratio(problem, modes[name])
                ...
                print(
                    f"[{index + 1}/100] {name} valid={valid} fallback={fallback_expected} "
                    f"speedup={speedup:.3f} max_abs={max_abs_error:.3e}",
                    flush=True,
                )

...
def verify_record(result: dict[str, object]) -> dict[str, object]:
    ...
    k = int(target["k"])
    t = int(target["t"])
    prior_upper = int(target["frozen_upper_bound"])
    ...
    expected_subsets = math.comb(v, t)
    if len(covered) != expected_subsets:
        problems.append(f"covers {len(covered)} of {expected_subsets} required t-subsets")
    ...
    return {
        ...
        "required_t_subsets": expected_subsets,
...
def main() -> None:
    ...
    invalid = [row for row in verifications if not row["valid"]]
    if invalid:
        problems.append("one or more claimed record candidates failed independent verification")

    report = {
        "protocol": "LEXIGEN World Covering Record v1 independent verification",

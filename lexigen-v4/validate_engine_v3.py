...
from engine_v3 import ENGINE_VERSION, fingerprint, generate_proposals
from selector import MIN_FAMILIES, TASK_COUNT, classify, select_tasks, task_score

OUTPUT = Path("validation-evidence-r3")
...
def is_solution(problem, solution):
    return np.allclose(solve(problem)["centers"], solution["centers"], atol=1e-6)
''',
    "convex_projection": '''
import numpy as np
...
EXPECTED_PRESENT = {
    ...
    "grouped_numeric": {"array", "cluster", "grouped_generator", "block_structure", "approximate_verifier"},
...
def validate() -> dict[str, object]:
    ...
    for label, source in SYNTHETIC_TASKS.items():
        ...
        require(EXPECTED_PRESENT[label].issubset(features), f"missing features for {label}: {sorted(EXPECTED_PRESENT[label] - features)}")
        require(not features.intersection(EXPECTED_ABSENT[label]), f"false-positive features for {label}: {sorted(features & EXPECTED_ABSENT[label])}")

        arms: dict[str, list[dict[str, object]]] = {}
        for arm in ("v4_full", "v4_no_transfer", "random_search", "template_synthesis", "v3_compatible"):
            proposals = generate_proposals(fp, arm=arm, limit=6, random_seed="validation-r3")
            repeated = generate_proposals(fp, arm=arm, limit=6, random_seed="validation-r3")
        ...
        require(REQUIRED_FULL_OPERATORS[label].issubset(full_operators), f"v4 omitted expected mechanisms for {label}: {sorted(REQUIRED_FULL_OPERATORS[label] - full_operators)}")
...
def main() -> None:
    ...
    try:
        report = validate()
        (OUTPUT / "engine-validation-r3.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        print(json.dumps({"engine_version": ENGINE_VERSION, "synthetic_tasks": len(SYNTHETIC_TASKS), "unique_proposal_ids": report["unique_proposal_ids"], "validation_sha256": report["validation_sha256"]}, indent=2))
    except Exception as exc:
        failure = {
            "engine_version": ENGINE_VERSION,
            "status": "failed_snapshot_free_validation",
            "error_type": type(exc).__name__,
            "error": str(exc),
            "traceback": traceback.format_exc(),
            "holdout_inventory_accessed": False,
            "task_contents_accessed": False,
        }
        (OUTPUT / "engine-validation-r3-failure.json").write_text(json.dumps(failure, indent=2) + "\n", encoding="utf-8")

...
def fetch(url: str) -> bytes:
    ...
    for attempt in range(8):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v4-task8-train-r1"})
...
def discover() -> tuple[str, str, str, str, bytes]:
...
def decode_problem(problem_raw: object) -> Problem:
    ...
    arr = np.asarray(arr)
    if arr.ndim != 2 or arr.shape[0] != arr.shape[1] or arr.size == 0 or not np.all(np.isfinite(arr)):
        raise RuntimeError(f"unexpected training matrix: shape={arr.shape} dtype={arr.dtype}")
    return arr
...
def flattened_candidates() -> list[tuple[str, str, Callable[[Problem], Solution]]]:
    ...
    if len(result) != 27:
        raise RuntimeError(f"expected 27 frozen candidates, got {len(result)}")
    return result
...
def main() -> None:
    ...
    train_name, train_oid, test_name, test_oid, raw = discover()

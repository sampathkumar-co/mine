...
MANIFEST = "unit_simplex_projection_T100ms_n982958_size100_test.jsonl"
EXPECTED_GIT_BLOB_SHA1 = "33358f9cad20ab1d145ccf51f63caaad091cc35f"
...
def fetch(url: str) -> bytes:
    ...
    for attempt in range(8):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v4-task6-blind-r1"})
...
def decode_problem(raw: object) -> Problem:
    ...
    if not isinstance(ref, dict) or ref.get("__type__") != "ndarray_ref":
        raise RuntimeError("expected ndarray_ref y in official Task 6 blind manifest")
    relative = str(ref.get("npy_path", ""))
    if not relative or relative.startswith("/") or ".." in Path(relative).parts:
        raise RuntimeError(f"unsafe or missing blind npy_path: {relative}")
    ...
    if y.shape != (EXPECTED_N,) or y.dtype != np.float64 or not np.all(np.isfinite(y)):
        raise RuntimeError(f"unexpected blind y sidecar: shape={y.shape} dtype={y.dtype}")
    return {"y": y}
...
def validate(proposed: Solution | None, expected: Solution) -> tuple[bool, str | None, float, float]:
    ...
    try:
        candidate = np.asarray(proposed["solution"], dtype=np.float64).reshape(-1)
    except Exception:
        return False, "solution_decode", float("inf"), float("inf")
    ref = np.asarray(expected["solution"], dtype=np.float64).reshape(-1)
    if candidate.shape != (EXPECTED_N,) or candidate.shape != ref.shape or not np.all(np.isfinite(candidate)):
...
def selected_candidates() -> list[tuple[str, str, Callable[[Problem], Solution]]]:
    ...
    for arm, wanted in SELECTED.items():
        ...
        if len(matches) != 1:
            raise RuntimeError(f"selected candidate not unique: {arm}/{wanted}")

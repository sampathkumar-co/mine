...
def fetch(url):
    ...
    for attempt in range(8):
        try:
            with urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':'LEXIGEN-v5-task8-train-r1'}),timeout=240) as r:return r.read()
...
def decode_problem(p):
    ...
    X=np.asarray(p['data_points']);Q=np.asarray(p['query_points']);
    if X.ndim!=2 or Q.ndim!=2 or X.shape[0]==0 or X.shape[1]!=Q.shape[1]:raise RuntimeError('invalid KDE shapes')
    return p
def timed(fn,p):

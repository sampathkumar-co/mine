...
def summarise(rows,arm,candidate):
    ...
    speeds=[float(r['speedup']) for r in s]; valid=sum(bool(r['valid']) for r in s); retries=sum(int(r['invalid_output_retries']) for r in s); times=[float(r['candidate_s']) for r in s if r['candidate_s'] is not None]
    ...
    out['passes_training_correctness']=valid==100 and retries==0; out['passes_default_performance_gate_on_training']=bool(out['passes_training_correctness'] and out['harmonic_speedup']>=1.5 and out['minimum_speedup']>=1.05); return out
def select(ss,arm):
...
def main():
    ...
    per={i:0 for i in range(1,101)}
    for r in rows: per[int(r['index'])]=per.get(int(r['index']),0)+1
    if any(per.get(i)!=30 for i in range(1,101)): raise RuntimeError('record coverage mismatch')
    ...
    comparison={'v5_full_harmonic':full['harmonic_speedup'],'v5_no_transfer_harmonic':nt['harmonic_speedup'],'v5_over_no_transfer_ratio':ratio,'selected_v5_uses_learned_transfer':full['learned_transfer'] is not None,'selected_v5_implementation_class':full['implementation_class'],'selected_no_transfer_implementation_class':nt['implementation_class'],'selected_pair_semantically_distinct':full['implementation_class']!=nt['implementation_class'],'equivalent_implementation_available_in_no_transfer':any(CANDIDATE_META[n]['implementation_class']==full['implementation_class'] for n in names['v5_no_transfer']),'training_causal_separation_threshold_crossed':bool(full['learned_transfer'] is not None and full['implementation_class']!=nt['implementation_class'] and full['passes_training_correctness'] and (not nt['passes_training_correctness'] or ratio>=1.25)),'causal_transfer_credit':False}
    manifest={(r['train_manifest_name'],r['train_manifest_git_blob_sha1'],r['train_manifest_sha256'],r['expected_test_manifest_name'],r['expected_test_manifest_tree_oid'],r['expected_test_manifest_size']) for r in rows}
    if len(manifest)!=1: raise RuntimeError('manifest identity mismatch')
    tr_name,tr_oid,tr_sha,te_name,te_oid,te_size=next(iter(manifest))
    report={'campaign':'LEXIGEN v5 Causal Transfer Generalization Experiment','task_index':5,'task':'earth_movers_distance','revision':1,'stage':'official_training','train_manifest_name':tr_name,'train_manifest_git_blob_sha1':tr_oid,'train_manifest_sha256':tr_sha,'expected_test_manifest_name':te_name,'expected_test_manifest_tree_oid':te_oid,'expected_test_manifest_size':te_size,'training_records':100,'candidate_count':30,'frozen_default_gate':{'valid_required':100,'harmonic_speedup_minimum':1.5,'minimum_speedup':1.05,'invalid_output_retries':0},'all_candidates':ss,'arms':arms,'architecture_comparison':comparison,'blind_selection_ready':bool(arms['v5_full']['correct_candidate_count']),'official_test_manifest_contents_opened':False,'official_test_payloads_opened':0}
    a.output.mkdir(parents=True,exist_ok=True); (a.output/'training-summary.json').write_text(json.dumps(report,indent=2)+'\n'); (a.output/'training-results.jsonl').write_text('\n'.join(json.dumps(r,separators=(',',':')) for r in sorted(rows,key=lambda r:(int(r['index']),r['arm'],r['candidate'])))+'\n'); print(json.dumps({'selected_by_arm':{k:v['selected'] for k,v in arms.items()},'architecture_comparison':comparison},indent=2))

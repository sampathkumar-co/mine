...
def fetch(relative_path: str, *, optional: bool = False) -> bytes | None:
    ...
    req = urllib.request.Request(f"{RAW_BASE}/{relative_path}", headers={"User-Agent": "LEXIGEN-v5-task4-source-r1"})
    try:
...
def main() -> None:
    ...
    if start["task"] != TASK or int(start["task_index"]) != TASK_INDEX or start["source_commit"] != SOURCE_COMMIT:
        raise RuntimeError("Task 4 identity mismatch")
    forbidden = (
    ...
    if any(forbidden):
        raise RuntimeError("Task 4 staged boundary was not clean before source access")
    ...
    if source_raw is None:
        raise RuntimeError("required Task 4 source is missing")
    source_blob = git_blob_sha1(source_raw)
    if source_blob != str(start["expected_source_git_blob_sha1"]):
        raise RuntimeError(f"Task 4 source blob mismatch: {source_blob}")

    description_raw = fetch(description_path, optional=True)
    if description_raw is not None:
        description_blob = git_blob_sha1(description_raw)
        if description_blob != str(start["expected_description_git_blob_sha1"]):
            raise RuntimeError(f"Task 4 description blob mismatch: {description_blob}")
    else:
    ...
    report = {
        ...
        "description_git_blob_sha1": description_blob,

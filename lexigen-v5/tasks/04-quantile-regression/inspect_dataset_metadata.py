...
def main() -> None:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v5-task4-metadata-only-r1"})
    ...
    for item in payload:
        ...
        if not path.startswith(f"data/{TASK}/"):
            raise RuntimeError(f"unexpected path outside Task 4 dataset directory: {path}")
        rows.append({
            "path": path,
            "type": item.get("type"),
            "size": item.get("size"),

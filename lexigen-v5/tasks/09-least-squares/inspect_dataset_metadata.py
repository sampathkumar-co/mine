...
REVISION='bb02811fa47ca1c833baaa344949bcd8fb307ac8';TASK='least_squares'
...
def main():
    ...
    with urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':'LEXIGEN-v5-task9-metadata-r1'}),timeout=180) as r:payload=json.loads(r.read().decode())
    rows=[]
    for x in payload:
        p=str(x.get('path',''))
    ...
    report={'campaign':'LEXIGEN v5 Causal Transfer Generalization Experiment','task_index':9,'task':TASK,'stage':'dataset_directory_metadata_r1','dataset_revision':REVISION,'training_manifest_metadata':tr[0],'test_manifest_metadata':te[0],'manifest_contents_opened':False,'payload_contents_opened':False,'resolve_or_download_urls_followed':False}

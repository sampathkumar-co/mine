...
def score_prediction(prediction: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    if prediction.get("status") != "frozen_before_R4_performance_spec_timing_or_outcome":
        raise ValueError("prediction is not an R4 prospective freeze")
    if prediction.get("R4_performance_spec_accessed") or prediction.get("R4_outcome_accessed") or prediction.get("R4_timing_accessed"):
        raise ValueError("prediction freeze records forbidden R4 access")
    if prediction.get("R4_ground_truth_message_or_diff_accessed"):
    ...
    if str(result.get("status", "")).startswith("infrastructure_"):
        return {
            ...
            "stage": "prospective_exact_sequence_transfer_score",

...
def select_targets(coverdata: dict[str, object]) -> tuple[list[Target], list[Target]]:
    ...
    needed = V1_RESERVED_COUNT + TARGET_COUNT
    for target in eligible:
        ...
        if len(selected) == needed:
            break
    if len(selected) != needed:
        raise RuntimeError(f"selector found only {len(selected)} of {needed} required targets")
    return selected[:V1_RESERVED_COUNT], selected[V1_RESERVED_COUNT:]

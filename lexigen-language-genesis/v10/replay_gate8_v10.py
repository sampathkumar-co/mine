from __future__ import annotations

import argparse
import hashlib
import importlib
import json
import sys
from pathlib import Path
from typing import Any

from constructive_arcgen_v10 import generate as generate_constructive
from portable_runtime_v10 import as_grid as portable_grid
from portable_runtime_v10 import execute_portable
from state_machine_runtime_v10 import as_grid, canonical_json, execute_machine, to_json_grid
from state_machine_synthesizer_v10 import synthesize_state_machine

ARCGEN_COMMIT = "a15cbdb44c776610aeeb9f487a06af875d3d0878"
TASK_ID = "6bcdb01e"


def load_redacted(path: Path):
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("selected_task_id") != TASK_ID:
        raise AssertionError("unexpected gate-8 task")
    examples = [(as_grid(item["input"]), as_grid(item["output"])) for item in payload["train"]]
    tests = [as_grid(item["input"]) for item in payload["test"]]
    return payload, examples, tests


def v9_baseline_fits(examples) -> bool:
    v9_root = Path(__file__).resolve().parents[1] / "v9"
    sys.path.insert(0, str(v9_root))
    try:
        module = importlib.import_module("object_motion_synthesizer_v9")
        result = module.synthesize_object_motion(examples)
        return result.extension is not None
    except (ValueError, RuntimeError, KeyError, IndexError, AssertionError):
        return False
    finally:
        if str(v9_root) in sys.path:
            sys.path.remove(str(v9_root))


def generate_holdout(arcgen_root: Path, start: int, count: int):
    sys.path.insert(0, str(arcgen_root))
    task = importlib.import_module("tasks.task_6bcdb01e")
    cases = []
    for offset, seed in enumerate(range(start, start + count), start=1):
        item = generate_constructive(task, seed)
        cases.append((seed, as_grid(item["input"]), as_grid(item["output"])))
        if offset % 1_000 == 0:
            print(json.dumps({"generated_holdout_cases": offset}), flush=True)
    return cases


def run(redacted: Path, arcgen_root: Path, output_dir: Path) -> dict[str, Any]:
    payload, examples, tests = load_redacted(redacted)
    result = synthesize_state_machine(examples)
    if result.machine is None:
        raise AssertionError("v10 failed to synthesize a state machine")
    machine = result.machine
    training_exact = all(execute_machine(machine, source) == target for source, target in examples)
    portable_training = all(execute_portable(machine, portable_grid(source)) == target for source, target in examples)
    v9_found = v9_baseline_fits(examples)
    print(
        json.dumps(
            {
                "candidates_tested": result.candidates_tested,
                "exact_candidates": result.exact_candidate_count,
                "machine": machine["name"],
            }
        ),
        flush=True,
    )

    holdout = generate_holdout(arcgen_root, 50_000, 10_000)
    failures = []
    portable_failures = []
    for offset, (seed, source, target) in enumerate(holdout, start=1):
        primary = execute_machine(machine, source)
        portable = execute_portable(machine, portable_grid(source))
        if primary != target:
            failures.append(seed)
        if portable != target or portable != primary:
            portable_failures.append(seed)
        if offset % 1_000 == 0:
            print(
                json.dumps(
                    {
                        "validated_holdout_cases": offset,
                        "primary_failures": len(failures),
                        "portable_failures": len(portable_failures),
                    }
                ),
                flush=True,
            )
    predictions = [to_json_grid(execute_machine(machine, source)) for source in tests]
    machine_sha = hashlib.sha256(canonical_json(machine).encode()).hexdigest()
    task_source = arcgen_root / "tasks" / "task_6bcdb01e.py"
    certificate = {
        "schema": "lexigen-v10-state-machine-certificate-v1",
        "machine_sha256": machine_sha,
        "task_source_sha256": hashlib.sha256(task_source.read_bytes()).hexdigest(),
        "training_exact": training_exact,
        "portable_training_exact": portable_training,
        "holdout_count": len(holdout),
        "holdout_exact": len(holdout) - len(failures),
        "portable_holdout_exact": len(holdout) - len(portable_failures),
        "v9_fixed_grammar_found": v9_found,
        "machine_ablation_training_exact": all(source == target for source, target in examples),
    }
    report = {
        "version": "v10",
        "benchmark": "ARC-GEN 6bcdb01e post-failure sensor-state-machine growth",
        "status": "conditional state-machine grammar candidate; not a blind breakthrough claim",
        "arcgen_commit": ARCGEN_COMMIT,
        "source_task_id": TASK_ID,
        "source_sealed_outputs_accessed": False,
        "candidate_machines_tested": result.candidates_tested,
        "exact_machine_count": result.exact_candidate_count,
        "generated_machine": machine,
        "certificate": certificate,
        "holdout_seed_range": [50_000, 59_999],
        "holdout_failures": failures,
        "portable_holdout_failures": portable_failures,
        "test_prediction_count": len(predictions),
        "claim_boundary": (
            "The machine is synthesized from generic state, sensor, vector and transition choices after gate 8 failed. "
            "A fresh sealed task is still required before any external breakthrough claim."
        ),
    }
    gate = {
        "new_production_absent_from_v9": not v9_found,
        "no_finished_task_operator": not machine["provenance"]["human_supplied_finished_task_operator"],
        "training_exact": training_exact,
        "portable_training_exact": portable_training,
        "holdout_exact": not failures,
        "portable_holdout_exact": not portable_failures,
        "ablation_fails": not certificate["machine_ablation_training_exact"],
        "sealed_outputs_untouched": True,
    }
    report["gate"] = gate
    if not all(gate.values()):
        raise AssertionError(f"v10 gate failed: {gate}")

    output_dir.mkdir(parents=True, exist_ok=True)
    machine_path = output_dir / "v10-state-machine.json"
    report_path = output_dir / "v10-report.json"
    predictions_path = output_dir / "gate8-postfailure-predictions.json"
    machine_path.write_text(json.dumps(machine, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    predictions_path.write_text(json.dumps({"predictions": predictions}, indent=2) + "\n", encoding="utf-8")
    summary = {
        "machine": machine["name"],
        "candidates_tested": result.candidates_tested,
        "exact_machine_count": result.exact_candidate_count,
        "holdout_accuracy": certificate["holdout_exact"] / certificate["holdout_count"],
        "v9_baseline_found": v9_found,
        "report_sha256": hashlib.sha256(report_path.read_bytes()).hexdigest(),
    }
    print(json.dumps(summary, indent=2, sort_keys=True))
    return report


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--redacted", type=Path, required=True)
    parser.add_argument("--arcgen-root", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/v10"))
    args = parser.parse_args()
    run(args.redacted, args.arcgen_root, args.output_dir)


if __name__ == "__main__":
    main()

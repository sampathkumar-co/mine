...
def verify_against_reference_portable(
    ...
    if contract.get("schema") != "lexigen-v17-verifier-contract-v1":
        return False
    if contract.get("verifier_grammar_sha256") != VERIFIER_GRAMMAR_SHA256:
        return False
    if not all(isinstance(contract.get(name), str) and len(contract[name]) == 64 for name in required_hashes):
        return False
...
def verify_output_portable(
    ...
    reference = execute_portable(program, source)

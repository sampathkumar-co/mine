...
def infer_recolour(examples: Sequence[tuple[Grid, Grid]]) -> dict[str, Any] | None:
    ...
    return {"op": "recolour", "mapping": {str(k): v for k, v in sorted(mapping.items())}} if mapping else None
...
def candidate_stages(examples: Sequence[tuple[Grid, Grid]]) -> tuple[dict[str, Any], ...]:
    ...
    common_singletons = set.intersection(*(singleton_colours(source) for source, _ in examples))
    ...
    if all(shape(target) == (shape(source)[0] + 2, shape(source)[1] + 2) for source, target in examples):
        fill_values = {target[0][0] for _, target in examples}
        if len(fill_values) == 1:
            stages.append({"op": "edge_project", "radius": 1, "fill_colour": next(iter(fill_values))})

    all_colours = colours(grid for pair in examples for grid in pair)
    for line_colour in list(all_colours) + ["structural"]:
    ...
    in_h, in_w = shape(first_source)
    out_h, out_w = shape(first_target)
    if out_h and out_w and in_h % out_h == 0 and in_w % out_w == 0:
        tile_rows, tile_cols = in_h // out_h, in_w // out_w
        tile_count = tile_rows * tile_cols
        if 2 <= tile_count <= 6 and all(
            shape(source) == (out_h * tile_rows, out_w * tile_cols)
            and shape(target) == (out_h, out_w)
            for source, target in examples
        ):
    ...
    for name in TRANSFORMS[1:]:
        stages.append({"op": "transform", "name": name})
    unique = {canonical(stage): stage for stage in stages}
    return tuple(unique[key] for key in sorted(unique, key=lambda text: hashlib.sha256(text.encode()).digest()))
...
def pipeline_key(pipeline: Pipeline) -> tuple[int, bytes]:
    ...
    return sum(stage_cost(stage) for stage in pipeline), hashlib.sha256(text.encode()).digest()
        ...
        for signature, pipeline in sorted(frontier.items(), key=lambda item: pipeline_key(item[1])):
            for stage in inventory:
                ...
                if tested > candidate_budget:
                    ...
                    return SceneSynthesisResult(chosen, tested - 1, len(visited), len(inventory), len(exact))
                try:
...
# Refine edge projection by inferring the destination canvas colour.
_candidate_stages_without_canvas_colour = candidate_stages


def candidate_stages(examples: Sequence[tuple[Grid, Grid]]) -> tuple[dict[str, Any], ...]:
    stages = [
        stage
        for stage in _candidate_stages_without_canvas_colour(examples)
        if stage.get("op") != "edge_project"
    ]
    if all(shape(target) == (shape(source)[0] + 2, shape(source)[1] + 2) for source, target in examples):
        corner_values = {
            value
...
# Prefer structural separator roles over literal separator colours when possible.
_candidate_stages_before_structural_roles = candidate_stages


def candidate_stages(examples: Sequence[tuple[Grid, Grid]]) -> tuple[dict[str, Any], ...]:
    stages = list(_candidate_stages_before_structural_roles(examples))
    for name in TRANSFORMS:
        stages.append({
            "op": "decode_regular_linegrid",

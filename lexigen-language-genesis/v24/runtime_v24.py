...
def transform_single_point(
    ...
    if name == "grid_rotate180":
        return height - 1 - row, width - 1 - col
    if name == "bbox_reflect_left":
        return row, 2 * min_col - 1 - col
    if name == "bbox_reflect_right":
        return row, 2 * max_col + 1 - col
    if name == "bbox_reflect_top":
        return 2 * min_row - 1 - row, col
    if name == "bbox_reflect_bottom":
        return 2 * max_row + 1 - row, col
    raise RuntimeV24Error(f"unknown transform: {name}")
...
def execute_paint_edit(program: dict[str, Any], source: Grid) -> Grid:
    ...
    values = (
        ...
        if program["base_mode"] == "input"
        else blank_like(source)
    )
...
def execute_relational_classify(program: dict[str, Any], source: Grid) -> Grid:
    ...
    for point in selected:
        related_point = transform_single_point(
            point,
            relation,
            height,
            width,
        ...
        values[point[0]][point[1]] = (
            equal_colour if related_point in selected else unequal_colour
        )
    return tuple(tuple(row) for row in values)

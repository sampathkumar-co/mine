...
def main() -> None:
    ...
    for path in sorted(args.reports_root.rglob("*.json")):
        report = load(path)
        if report.get("schema") != "lexigen-v28-task-scan-v1":
            continue
    ...
    for structure_hash in sorted(structure_index):
        ...
        document = {
            "schema": "lexigen-v28-factorized-structure-v1",
    ...
    library = {
        "schema": "lexigen-v28-factorized-library-v1",
    ...
    report = {
        "schema": "lexigen-v28-discovery-report-v1",
        "precommit_sha256": sha256_file(precommit_path),
        "discovery_task_count": len(expected_ids),

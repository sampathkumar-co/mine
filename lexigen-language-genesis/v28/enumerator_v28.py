...
def enumerate_programs(
    ...
    *,
    ...
    latest_useful_generation_depth: dict[str, int],
    retain_nonexact_at_maximum_grid_depth: bool,
) -> dict[str, Any]:
    if not examples:
    ...
    for depth in range(1, maximum_depth + 1):
        if stop:
            break
        for type_name in TYPE_ORDER:
            if depth > int(latest_useful_generation_depth[type_name]):
                stats.append({"depth": depth, "type": type_name, "retained_expressions": 0, "unique_considered": 0, "raw_candidates": 0, "runtime_invalid": 0, "semantic_duplicates": 0, "beam_dropped": 0, "skipped_reason": "liveness_pruned"})
                continue
            if type_name == "Color":
                stats.append({
                    "depth": depth,
            ...
            if type_name == "Grid" and depth == maximum_depth and not retain_nonexact_at_maximum_grid_depth:
                keep = 0
    ...
    return {
        "schema": "lexigen-v28-liveness-pruned-enumeration-result-v1",

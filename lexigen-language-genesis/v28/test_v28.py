...
from enumerator_v28 import enumerate_programs, grid_distance, support_distance, target_anchors
...
def run(examples, *, grid_beam=20):
    return enumerate_programs(
        ...
        latest_useful_generation_depth={"Color": 0, "ObjectSet": 2, "PointSet": 2, "Grid": 3},
        retain_nonexact_at_maximum_grid_depth=False,
    )
...
def test_liveness_prunes_dead_frontiers():
    result = run([paint_border_example(8)], grid_beam=5)
    object_depth_three = next(
        item for item in result["statistics"]
        if item["depth"] == 3 and item["type"] == "ObjectSet"
    )
    point_depth_three = next(
        item for item in result["statistics"]
        if item["depth"] == 3 and item["type"] == "PointSet"
    )
    grid_depth_three = next(
        item for item in result["statistics"]
        if item["depth"] == 3 and item["type"] == "Grid"
    )
    assert object_depth_three["skipped_reason"] == "liveness_pruned"
    assert point_depth_three["skipped_reason"] == "liveness_pruned"
    assert grid_depth_three["retained_expressions"] == 0

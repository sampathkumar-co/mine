...
from enumerator_v28 import enumerate_programs
...
def main() -> None:
    ...
    precommit_path = HERE / "V28_PRECOMMIT.json"
    ...
    if len(examples) != int(precommit["examples_per_task"]):
        report = {
            "schema": "lexigen-v28-task-scan-v1",
        ...
        enumeration = enumerate_programs(
            examples,
            maximum_depth=int(precommit["search"]["maximum_depth"]),
            maximum_unique_considered_per_type_per_depth=int(
                precommit["search"]["maximum_unique_considered_per_type_per_depth"]
            ),
            maximum_raw_candidates=int(
                precommit["search"]["maximum_raw_candidate_evaluations_per_task"]
            ),
            retained_total_caps={k: int(v) for k, v in precommit['search']['retained_total_caps'].items()},
            beam_per_depth={k: int(v) for k, v in precommit['search']['beam_per_depth'].items()},
            latest_useful_generation_depth={k: int(v) for k, v in precommit['search']['latest_useful_generation_depth'].items()},
            retain_nonexact_at_maximum_grid_depth=bool(precommit['search']['retain_nonexact_at_maximum_grid_depth']),
        )
        report = {
            "schema": "lexigen-v28-task-scan-v1",
            "task_id": args.task_id,
        ...
        summary.update(
            {
                ...
                "unique_expressions": report["enumeration"]["total_retained_expressions"],

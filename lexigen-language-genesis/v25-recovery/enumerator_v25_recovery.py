...
class ExactSeenStore:
    def __init__(self) -> None:
        fd, name = tempfile.mkstemp(prefix='lexigen-v25-seen-', suffix='.sqlite3')
        os.close(fd)
        self.path = name
        ...
        self.connection.execute('CREATE TABLE seen (namespace TEXT NOT NULL, digest BLOB NOT NULL, payload BLOB NOT NULL, PRIMARY KEY(namespace,digest,payload)) WITHOUT ROWID')

    def add_payload(self, namespace: str, payload: bytes) -> bool:
        digest = hashlib.sha256(payload).digest()
        cursor = self.connection.execute('INSERT OR IGNORE INTO seen(namespace,digest,payload) VALUES(?,?,?)', (namespace,digest,payload))
        return cursor.rowcount == 1

    def add_signature(self, type_name: str, signature: tuple[Any, ...]) -> bool:
        return self.add_payload(type_name, pickle.dumps(signature, protocol=5))
...
def make_expression(
    ...
    return Expression(
        ...
        depth=depth,
        nodes=nodes,
        ast_text=canonical(ast) if ast_text is None else ast_text,
        values=values,
    )
    ...
    *,
    ...
    for type_name in TYPE_ORDER:
        ...
        for expression in leaves[type_name]:
            ...
            if not seen.add_signature(type_name, signature):
        ...
        for type_name in TYPE_ORDER:
            ...
            for candidate in merge_streams(streams):
                ...
                if raw_candidates >= maximum_raw_candidates:
                    ...
                    stop = True
                    break
                if not seen.add_payload('__ast__', candidate.ast_text.encode('utf-8')):
                    ast_duplicates += 1
                    continue
                raw_candidates += 1
                try:

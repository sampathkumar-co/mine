...
def main()->None:
    ...
    report={'schema':'lexigen-v19r2-fixed-pool-report-v1','base_rejected_v19_commit':pre['base_rejected_v19_commit'],'gates_fixed':len(outcomes),'cases_per_success':args.cases,'productions_found':sum(x['status']=='production_found' for x in outcomes),'no_program_failures':sum(x['status']=='no_program' for x in outcomes),'hidden_outputs_opened':False,'world_level_breakthrough':False,'claim_boundary':{'autonomous_grammar_production_candidate':False,'automated_argument_abstraction_candidate':not any_fresh_failure and any(x['status']=='production_found' for x in outcomes),'fixed_affine_template_human_supplied':True,'sealed_external_success':False,'transfer_demonstrated':False,'reason':'The exact affine composition skeleton was human-authored after demonstration analysis; automation selected arguments and serialized an abstraction but did not invent the skeleton.'},'outcomes':outcomes}

...
def fresh_cases(task:str,gate:int,root:Path,count:int):
    ...
    while len(cases)<count:
        seed=7_900_000+gate*100_000+attempts; attempts+=1
...
def main()->None:
    ...
    production=load(HERE/'productions'/'gate-10-production.json'); program=load(HERE/'productions'/'gate-10-concrete.json')
    primary_hash=fsha(HERE/'runtime_v19r2.py'); portable_hash=fsha(HERE/'portable_runtime_v19r2.py'); demo_hash=sha256_json([{'input':s,'output':t} for s,t in examples])

...
VERIFIER_GRAMMAR_SHA256 = _sha(CATALOG)
...
def verify_against_reference_portable(
    ...
    if contract.get("schema") != "lexigen-v19r2-verifier-contract-v1":
        return False
    if contract.get("verifier_grammar_sha256") != VERIFIER_GRAMMAR_SHA256:
        return False
    if contract.get("contract_sha256") != _sha(_payload(contract)):
        return False
    required = ("production_sha256", "concrete_program_sha256", "primary_runtime_sha256", "portable_runtime_sha256", "demonstration_sha256")
    if not all(isinstance(contract.get(name), str) and len(contract[name]) == 64 for name in required):
        return False
    if contract.get("production_sha256") != _sha(production):
        return False
    if contract.get("concrete_program_sha256") != _sha(concrete_program):
        return False
...
def verify_output_portable(
    ...
    reference = execute_portable(concrete_program, source)
    return verify_against_reference_portable(

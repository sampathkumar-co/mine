...
from latent_runtime_v13_ext2 import execute_program
from latent_synthesizer_v13 import LatentSynthesisResult, canonical_json, colours, program
from latent_synthesizer_v13_ext import candidate_programs as previous_candidates
...
def candidate_programs(examples: Sequence[tuple[Grid, Grid]]):
    ...
    if same_shape:
        for noise_colour in values:
            yield program("reconstruct_periodic_lattice", noise_colour=noise_colour)
        for key_colour in values:
            yield program("template_colour_broadcast_invariant_v2", key_colour=key_colour)

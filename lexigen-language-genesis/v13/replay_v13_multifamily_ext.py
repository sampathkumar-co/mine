...
from latent_synthesizer_v13_ext import synthesize_latent
...
def run(evidence_root: Path, arcgen_root: Path, holdout_count: int) -> None:
    ...
    for gate_index, gate in enumerate(GATES):
        ...
        training_exact = bool(generated and all(execute_program(generated, source) == target for source, target in examples))
        failures = []
        if generated:
            generator, _ = registry[payload["selected_task_id"]]
            for offset in range(holdout_count):
                seed = 260_000 + gate_index * 20_000 + offset
                random.seed(seed)
                item = generator()

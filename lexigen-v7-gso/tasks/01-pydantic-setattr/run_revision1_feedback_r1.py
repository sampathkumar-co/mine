...
def main() -> None:
    ...
    for candidate in CANDIDATES:
        name = f'lexigen-v7-pydantic-revision1-{candidate.lower()}'
        ...
        try:
            ...
            for index in TEST_INDEXES:
                dexec(name, f"cd /testbed && source .venv/bin/activate && python /gso_test_{index}.py /tmp/candidate_{index}.txt --eqcheck --file_prefix /tmp/v7_revision1_ref_{index}")
        ...
        row = {
            ...
            'arm': 'v7_full' if candidate == 'F1' else ('v7_no_library' if candidate == 'N2' else 'v7_random_library'),
    ...
    report = {
        'stage': 'task1_revision_slot1_14_test_feedback_r1',
        'image': IMAGE,
        ...
        'candidates': rows,
        'revision_slot': 1,
        'revision_slots_consumed_per_arm': 1,
        'revision_slots_remaining_per_arm': 1,

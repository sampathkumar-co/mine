...
def main() -> None:
    ...
    if len(rows) != len(candidates) * 100:
        raise RuntimeError(f"expected {len(candidates) * 100} rows, received {len(rows)}")
    ...
    for candidate in candidates:
        ...
        summaries.append({
            ...
            "exceptions": sum(row["failure_reason"] not in (None, "output_mismatch") for row in selected),
    ...
    eligible = [
        ...
        if row["valid"] == 100
        and float(row["harmonic_speedup"]) >= 1.50
        and float(row["minimum_speedup"]) >= 1.05
    ]
    eligible.sort(key=lambda row: (-float(row["harmonic_speedup"]), str(row["candidate"])))
    selected = eligible[0] if eligible else max(
        summaries,
        key=lambda row: (int(row["valid"]), float(row["harmonic_speedup"]), float(row["minimum_speedup"])),
    )

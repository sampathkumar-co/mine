...
EXPECTED_GIT_BLOB_SHA1 = "194db2639d41ee0c91b2f171932e7984ad4aed3a"
...
def request_bytes(url: str) -> tuple[bytes, int]:
    ...
    for attempt, delay in enumerate(delays, start=1):
        if delay:
            time.sleep(delay)
        request = urllib.request.Request(url, headers={"User-Agent": "LEXIGEN-v3-task1-blind"})
        try:
...
def validate(
    ...
    try:
        ...
        return (
            ...
            candidate_distance,
            objective_ratio,
            None if valid else "feasibility_or_objective",
        )
    except Exception as exc:
...
def main() -> None:
    ...
    raw, manifest_attempts = request_bytes(f"{BASE}/{MANIFEST}?download=true")
    actual_blob = git_blob_sha1(raw)
    if actual_blob != EXPECTED_GIT_BLOB_SHA1:
        raise RuntimeError(f"blind manifest object mismatch: {actual_blob}")
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for index, row in selected:
            ...
            payload_bytes, payload_attempts = request_bytes(f"{BASE}/{relative}?download=true")
            ...
            valid, cvar, distance, objective_ratio, validation_reason = validate(
            ...
            record = {
                ...
                "candidate_retries": 0,
                "manifest_download_attempts": manifest_attempts,
                "payload_download_attempts": payload_attempts,
                "manifest_content_sha256": hashlib.sha256(raw).hexdigest(),
            }
            records.append(record)
            print(
                f"[{index + 1}/100] pruned_hybrid6 valid={valid} speedup={speedup:.3f} "

...
def main() -> None:
    ...
    if indices != list(range(1, 101)):
        raise RuntimeError(f"blind rows are not exactly records 1..100: {indices}")
    ...
    if any(str(row.get("test_manifest_tree_oid")) != EXPECTED_TREE_OID for row in rows):
        raise RuntimeError("test manifest identity changed")

    manifest_hashes = sorted({str(row["manifest_sha256"]) for row in rows})
    if len(manifest_hashes) != 1:
        raise RuntimeError(f"multiple test manifest content hashes observed: {manifest_hashes}")
    ...
    objective_ratios = [float(row["objective_ratio"]) for row in rows]
    cvar_violations = [float(row["cvar"]) - float(row["kappa"]) for row in rows]
    summary = {
        ...
        "maximum_objective_ratio": max(objective_ratios),
        "maximum_cvar_violation": max(cvar_violations),
    ...
    verification = {
        ...
        "indices_exact": indices == list(range(1, 101)),
        "candidate_identity_exact": all(str(row["candidate"]) == CANDIDATE for row in rows),
        "one_execution_each": all(
            int(row["candidate_executions"]) == 1 and int(row["reference_executions"]) == 1
            for row in rows
        ),
    }
    ...
    if not summary["passes_blind_gate"]:
        raise SystemExit(1)
